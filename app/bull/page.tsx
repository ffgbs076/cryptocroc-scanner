import { Coin, FuturesMeta, BtcContext, CandleMeta, usd, pct, scoreBull } from "../lib/scoring";
...
export default async function BullPage() {
  let coins: Coin[] = [];
  ...
  const pre = coins
    ...
    .map(c => {
      ...
      // eerst snelle score (zonder candles), daarna enrich top
      const scored = scoreBull(c, fut, btc);
      return { coin: c, baseScore: scored.score, fut };
    })
    .sort((a, b) => b.baseScore - a.baseScore);

  // ✅ hier doen we “verwachting”: candles + long/short ratio
  const ENRICH = 30;
  ...
  for (const item of shortlist) {
    ...
    const scored = scoreBull(c, fut, btc, candles);
    ...
    <div style={{ background: "black", color: "white", minHeight: "100vh", padding: 24 }}>
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 900 }}>BULL</h1>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <a href="/bear" style={{ color: "white", textDecoration: "none", border: "1px solid #333", padding: "10px 14px", borderRadius: 8 }}>Bear →</a>
          </div>
        </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {rows.map((r: any) => {
                  const c: Coin = r.coin;
                  const scoreColor = r.score >= 80 ? "#22c55e" : "white";
                  const fundingColor = r.fundingPct !== null && r.fundingPct < 0 ? "#22c55e" : "#b3b3b3";
                  const lsrColor = r.lsr !== null && r.lsr < 1 ? "#22c55e" : "#b3b3b3";
                  ...
                  return (
                    <tr key={c.id} style={{ borderTop: "1px solid #222" }}>
                      ...
                      <td style={{ padding: 12, color: "#22c55e", fontSize: 12 }}>{r.reasons.join(" • ")}</td>
                    </tr>
                  );
                })}
              </tbody>
